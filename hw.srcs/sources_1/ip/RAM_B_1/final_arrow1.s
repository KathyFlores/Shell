...
ori $2, $1, 4800 # end address of vram

lui $27, 0xf000

clean_vga0:
add $12, $1, $zero # initial t3, $11
llloop0: # clean vga
addi $9, $zero, 0x0
sw $9, 0($12)
addi $12, $12, 1
...
j initial
initial:
lui $a0, 0xd000 # address of keyboard, $4
lui $a1, 0x000c # begin addr of vram, $5
ori $s3, $a1, 4800 # end addr of vram, $19
lui $s5, 0xe000 # address of 7 seg, $21
add $t9, $zero, $zero # random number, $25
add $at, $zero, $zero # score, 
sw $at, 0($s5)
ori $v0, $a1, 20  # start addr of brick, 1st row, $2
# addi $v1, $v0, 3 # end addr of the brick, 1st row
ori $a2, $a1, 4740 # bottom bound of brick, $6
ori $s4, $a1, 4424 # start addr of board, $20
addi $s0, $zero, 0x100 # button left, $16
addi $s1, $zero, 0x400 # button right, $17
addi $s2, $zero, 0x0 # color white, $18
...
sw $s2, 3329($a1)
read_kbd2:
lw $t0, 0($a0) # load word from ps2, $8
andi $t1, $t0, 0x100 # state of switch[0]
beq $t1, $zero, start_background # get ready bit
andi $t1, $t0, 0xff
addi $30, $0, 0xf0
beq $t1, $30, lock2
...
lw $t0, 0($a0) # load word from ps2, $8
andi $t1, $t0, 0x100 # state of switch[0]
beq $t1, $zero, game_0 # get ready bit
andi $t1, $t0, 0xff
addi $30, $0, 0xf0
beq $t1, $30, lock3
#esc:
...
lw $t0, 0($a0) # load word from ps2, $8
andi $t1, $t0, 0x100 # state of switch[0]
beq $t1, $zero, e0key_ # get ready bit
andi $t1, $t0, 0xff
addi $30, $0, 0xf0
beq $t1, $30, lock3
...
addi $s4, $s4, -5
ori $t3, $a1, 4400
slt $t4, $s4, $t3 # s4<4400, t4=1, go to the most right
beq $t4, $zero, game
ori $s4, $a1, 4467
j game
...
addi $s4, $s4, 5
addi $t3, $a1, 4468
slt $t4, $s4, $t3 #s4<4468, t4=1, jump to game
bne $t4, $zero, game
addi $s4, $a1, 4400
j game
...
lw $t0, 0($a0) # load word from ps2, $8
andi $t1, $t0, 0x100 # state of switch[0]
beq $t1, $zero, lock2 # get ready bit
andi $t1, $t0, 0xff
addi $30, $0, 0xf0
bne $t1, $30, read_kbd2
j lock2
...
lw $t0, 0($a0) # load word from ps2, $8
andi $t1, $t0, 0x100 # state of switch[0]
beq $t1, $zero, lock3 # get ready bit
andi $t1, $t0, 0xff
addi $30, $0, 0xf0
bne $t1, $30, read_kbd3
j lock3